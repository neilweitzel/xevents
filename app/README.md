...
Optional browser regression (requires Playwright in the development environment
and a local static server):

```sh
node app/tests/browser-smoke.mjs http://127.0.0.1:3000
```

There is no application build step, server component, package installation,
credential, browser storage, telemetry or external network dependency. Hash
...
- Production G5 enforcement and existing limits remain unchanged.
...
- Desktop and 375px mobile layouts, all pages, long sector labels and table scroll.
...
On 2026-09-23: 12 model tests passed; Chromium interactions and actual downloaded
