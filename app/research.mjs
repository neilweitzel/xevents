...
function methodology() {
  $("#view").innerHTML = heading("Know what the numbers mean", "Claims are observations to examine, not breaches to declare.") +
    ...
    <h2>An automatic, bounded pipeline</h2><p>The collector is scheduled every two hours, with at least six hours between successful source captures and up to 100 recent records requested per capture so that delayed or skipped runs do not lose listings. Only listing metadata is collected; screenshots are not. It preserves evidence privately, groups repeat claims, checks eligibility and sector classification, scans the exact output, then signs and verifies each release before deployment. Ordinary eligible data does not wait for human review. Failures preserve the last valid site; exceptions stay withheld.</p>
    <h2>Retention</h2><p>Detailed private records are kept for at least twelve weeks. When every week of a month is older than that, the month is frozen: its published weekly and monthly counts stay exactly as they were, and the private records behind them are removed. Frozen counts cannot be recalculated, only withheld. The claims-assessed band keeps counting across frozen months.</p>
    ...
    <h2>Attribution</h2><p>Derived source: <a href="https://www.ransomlook.io/" rel="noreferrer">RansomLook</a>, <a href="https://www.ransomlook.io/about" rel="noreferrer">CC BY 4.0</a>. xevents supplies the grouping and sector aggregation. These are listing claims, not confirmed breaches.</p><p>xevents follows every source licence as we read it, and takes real care to do so. Mistakes can still happen. If you think we have read your terms wrong, <a href="https://github.com/neilweitzel/xevents/issues/new">report a bug</a> or open a pull request and it will be corrected immediately. Sources are removed or restricted on request.</p>
