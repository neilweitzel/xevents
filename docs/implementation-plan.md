...
| AC1.4 | CI | Every quarantine fixture appends exactly one correction-ledger entry with `event_type: administrative_note`, `authority: g5-machinery`, referencing the failing fixture id, the match rule, and the denylist version. |
...
| AC1.7 | CI | The `xevents-boundary` App scopes are exactly the minimum required by ADR 0012 §App configuration. A test enumerates the App's declared scopes and fails if any additional scope is present. |
