...
**Status:** accepted, 2026-09-22. Sub-task-level execution checklist
...
- [ ] Both repos scaffolded per `xevents-internal/docs/file-layout.md`
      ...
      thirteen ADRs; `ls docs/` shows the accepted-decision docs.
- [ ] All ADRs 0001–0013 accepted (Nygard `Status: accepted` line
      present). **Verify:** `grep -c '^- Status: accepted$' docs/adr/*.md`
      returns 13 (0008 excluded — it is `superseded by ADR 0009`).
...
scope set from ADR 0012 §Mechanics.
...
**AC coverage:** AC1.7 (scope minimum), AC1.16 (operator-verified
installation and key placement), ACP1.4 (secret placement),
ACP1.7 (rotation runbook).
...
- [ ] **Fixture 13 — Boundary write-set violation:** aggregates
      clean (G5 would pass); synthetic push instruction targets
      `docs/` (a fourth path outside the boundary write set).
      Expected `outcome: refused_by_branch_protection,
      api_response_code: 4xx, g5_ran: false`.
...
- [ ] Implement `g5/report.py` — writes the G5 report to
  ...
  - [ ] `outcome`: `pass` | `quarantine` | `refused_by_branch_protection`.
...
- [ ] Add a conditional step (only when G5 outcome is `pass`) that:
  ...
  - [ ] Uses the token to push the three boundary-writable files
        (`data/aggregates/*.jsonl`, `evidence-manifest.jsonl`,
        `coverage-boundary-statement.md`) to `xevents` `main`.
  - [ ] Uses the token's `actions: write` scope to dispatch
        `build-site.yml` on the public repo (per ADR 0012
        §Mechanics).
...
(trivial-pass push), AC1.2 (quarantine no-push), AC1.9 (machinery-
invariants report on every run).
...
Purpose: branch protection on `xevents` `main` that enforces
manual-vs-App commit distinction and the three-path boundary write
set. Per ADR 0012 §Identity and audit.

- [ ] Confirm signed commits are configured on the operator's
      local machine (`git config commit.gpgsign true` or SSH-key
      signing).
- [ ] `xevents` repo Settings → Rules → Rulesets: create **Ruleset
      P (PR + signature)** targeting `main` with:
  - [ ] `Require signed commits` on. `Require a pull request
        before merging` on. `Restrict deletions` on.
  - [ ] Bypass list: `xevents-boundary[bot]` (App, `always`).
        Rationale: the App pushes commits directly to `main` as
        part of the boundary workflow — no PR is possible for
        machine-driven pushes. The App's paths are constrained
        by Ruleset A (below); the operator's manual writes to
        `main` go through PRs and are constrained by Ruleset B
        (below).
  - [ ] Effect: operator changes land via PR with signed commits;
        the App's direct pushes are allowed but path-restricted
        by Ruleset A.

### Boundary path enforcement

The plan requires two mechanically enforced constraints:

- The **App** (`xevents-boundary[bot]`) can only touch the three
  boundary paths on `main`.
- The **operator** cannot manually touch the three boundary paths
  on `main`.

GitHub push rulesets enforce path restrictions on **everyone** by
default; specific actors get **bypass**. So two rulesets are
needed, one for each direction. `Restrict file paths` with
`Allowed exceptions` is used for both, following GitHub's push
ruleset syntax.

- [ ] **Ruleset A — App path allowlist** (Settings → Rules →
      Rulesets → New branch ruleset):
  - [ ] Target: branch `main`. Enforcement: `Active`.
  - [ ] Rule: `Restrict file paths`. `Restricted file paths`:
        `**/*` (everything). `Allowed exceptions`:
        `data/aggregates/**`, `evidence-manifest.jsonl`,
        `coverage-boundary-statement.md`.
  - [ ] Bypass list: `neilweitzel` (operator, `always`). The App
        is NOT on the bypass list.
  - [ ] Effect: the operator can push anywhere (they bypass); the
        App can only push the three boundary paths (it is subject
        to the rule).
- [ ] **Ruleset B — Operator manual-write block on boundary paths**
      (Settings → Rules → Rulesets → New branch ruleset):
  - [ ] Target: branch `main`. Enforcement: `Active`.
  - [ ] Rule: `Restrict file paths`. `Restricted file paths`:
        `data/aggregates/**`, `evidence-manifest.jsonl`,
        `coverage-boundary-statement.md`. No `Allowed exceptions`.
  - [ ] Bypass list: `xevents-boundary[bot]` (App, `always`). The
        operator is NOT on the bypass list.
  - [ ] Effect: the App can push the three boundary paths (it
        bypasses); the operator cannot push those three paths
        manually (they are subject to the rule).
- [ ] Test Ruleset B (operator manual write blocked): from a local
      branch, attempt to push a manual commit to `main` that
      modifies `data/aggregates/view1.jsonl`. Confirm the push is
      rejected. Screenshot the rejection and save to
      `xevents-internal/machinery-invariants/2026-09-22-ruleset-b-manual-reject.png`.
- [ ] Test Ruleset A (App non-boundary write blocked): fixture 13
      (ws11) exercises this path end-to-end — the App workflow
      attempts to push a fourth path (`docs/`) and the push is
      refused by Ruleset A.
- [ ] Write `xevents/docs/branch-protection.md` documenting both
      rulesets, with pointers to ADR 0012 §Boundary write set and
      to the M1 checklist entries that verified them. Include the
      screenshot and the fixture-13 CI run URL as evidence.

**AC coverage:** AC1.3 (branch protection API-layer refusal on
fixture 13). Enables AC1.1, AC1.2 (App pushes only when G5 passes,
only to boundary write set).
...
### Boundary-write-set assertion (AC1.8)
...
- [ ] Dispatch fixture 13 (write-set violation). Confirm:
  - [ ] Branch protection refuses the App push at the API layer
        (4xx response captured in workflow log).
  - [ ] Workflow exits non-zero.
  - [ ] Correction-ledger entry appended.
  - [ ] Machinery-invariants report emitted with
        `outcome: refused_by_branch_protection`.
...
**AC coverage:** AC1.1, AC1.2, AC1.3, AC1.4, AC1.5 (via fixtures
01, 02–12, 13, quarantine ledger entries, 14 respectively).
...
- `docs/adr/0012-aggregation-boundary-transport.md` — App scopes,
  boundary write set, branch-protection contract.
